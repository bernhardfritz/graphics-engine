...
export function subtract<T extends genType | mat2 | mat3 | mat4>(
  x: T | number,
  y: T | number
): T {
  if (isNumber(x)) {
    if (isNumber(y)) {
      ...
      return matn[y.length](...y.map((yy) => subtract(xx, yy))) as T;
    }
    ...
    if (isNumber(y)) {
      ...
      // y is matrix
      throw new Error('Cannot subtract vector and matrix');
    }
  } else {
    // x is matrix
    if (isNumber(y)) {
      const yy = genType[x[0].length](y);
      return matn[x.length](...x.map((xx) => subtract(xx, yy))) as T;
    } else if (isNumber(y[0])) {
      // y is vector
      throw new Error('Cannot subtract matrix and vector');
    } else {
      // y is matrix
